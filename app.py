...
async def _startup_event():
    ...
    async def _startup_mcp_connections():
        ...
        try:
            from src.integrations import reconcile_proven_native_mcp_companion_links

            linked = reconcile_proven_native_mcp_companion_links(
                mcp_manager.get_all_statuses()
            )
            if linked:
                logger.info(
                    "Grouped %d credential-proven legacy API connection(s) with native MCP",
                    linked,
                )
        except BaseException as e:
            logger.warning(
                "Proven native MCP companion reconciliation failed (non-critical): %s: %s",
                type(e).__name__,
                e,
            )
        try:
