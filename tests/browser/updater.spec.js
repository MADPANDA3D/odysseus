...
const FUTURE_WORKER = CURRENT_WORKER.replace('pandamonium-v411', 'pandamonium-v412');
...
for (const [scenario, bridgeReload] of [
  ...
  expect(FUTURE_WORKER).toContain("const CACHE_NAME = 'pandamonium-v412';");
  ...
  const sourceCache = bridgeReload ? 'pandamonium-v387' : 'pandamonium-v411';
  const targetCache = bridgeReload ? 'pandamonium-v411' : 'pandamonium-v412';
...
for (const [status, title] of [
  ...
  ['release_active', 'Installed release is active'],
    ...
    if (path === '/api/update/status') return route.fulfill({ json: {
      ...
      message: status === 'failed'
        ...
        : status === 'release_active'
          ? 'Installed release v1.0.21 is active; the privileged updater receipt is unavailable'
          : 'Rolled back to v1.0.20',
      rollback_available: false,
    } });
    return null;
  });
