...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "inspect_network": "Inspect the current network visible to Pandamonium with fixed read-only probes for interfaces, routes, neighbors, and optional Tailscale status. Use for requests to map, diagram, or explain the user's actual current network. Accepts no commands or paths and cannot mutate network state.",
    ...
    "ui_control": "Control the UI and toggle capability permissions on/off. Conversation routing is adaptive and has no user-selectable mode. ORACLE lifecycle only: `oracle_protocol engage` or `oracle_protocol shutdown`; active ORACLE map actions use its native tool catalog. Tool toggles: shell (bash), search (web), research, browser, documents, incognito. Open panels (documents library, gallery, email inbox, sessions, notes, memories/brain, skills, settings, cookbook) via `open_panel <name>`. Use `open_email_reply <uid> <folder> reply <body text>` (or structured body) to open an email reply draft document without sending. Also changes the current model and applies/creates themes.",
...
class ToolIndex:
    ...
    _KEYWORD_HINTS = {
        ...
        frozenset({"books library", "book library", "my books", "book catalog",
                   "source book", "book needs attention", "book needs ocr",
                   "from my book", "using my book"}):
            {"manage_books"},
