...
FUNCTION_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Manage calendar events: list events in a date range, create, update, delete. Each event can carry a tag/category (event_type) and importance level. Resolve relative dates like today/tomorrow against the 'Current date and time' system context, then pass ISO 8601 datetimes in the user's local wall time; for all-day events set all_day=true and pass YYYY-MM-DD. For event reminders/alarms, pass reminder_minutes; the tool creates the Pandamonium note reminder, so do not also call manage_notes for the same reminder. Do not set rrule for single-occurrence requests such as 'next Wednesday only'; use rrule only when the user explicitly wants recurrence.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {"type": "string",
                    ...
                    "reminder_minutes": {"type": "integer", "description": "For create_event: create an Pandamonium reminder this many minutes before the event, e.g. 5 for 'reminder 5 min before'."},
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Manage user preferences and settings. Use `disable_tool`/`enable_tool`/`list_tools` to turn individual tools on or off globally (e.g. shell, search, browser, documents, memory, skills, images, tasks, notes, calendar, email). Use `list_tools` to inspect the full built-in catalog, and `load_tools` to mount any enabled catalog entry for the rest of this request (the exact usage comes back in the result). Use list/get/set/delete for free-form preferences.",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            "name": "manage_books",
            "description": "Read the authenticated owner's private Books catalog or semantically search indexed book text with source title and page provenance. Use this for any request about 'my Books library', book indexing/OCR status, or facts from the user's books. Never search the filesystem for Books data.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {"type": "string", "enum": ["list", "search"], "description": "list catalog/status or search indexed book text"},
                    "query": {"type": "string", "description": "Semantic query required for search"},
                    "limit": {"type": "integer", "minimum": 1, "maximum": 12, "description": "Maximum source-aware chunks (default 6)"},
                },
                "required": ["action"],
                "additionalProperties": False,
            },
...
def function_call_to_tool_block(name: str, arguments: str) -> Optional[ToolBlock]:
    ...
    if tool_type == "bash":
        ...
        if action == "toggle":
            ...
            content = f"oracle_protocol {name or value}"
            if name and value:
                content += f" {value}"
